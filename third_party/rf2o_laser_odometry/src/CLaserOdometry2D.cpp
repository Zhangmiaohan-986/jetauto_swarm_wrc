                            ...
                            const geometry_msgs::Pose& initial_robot_pose)
{
  ...
  // Keep velocity timing in the sensor clock domain. Using ros::Time::now()
  // here can make the first increment negative or arbitrarily large when a
  // driver stamps scans before their callback reaches this process.
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  // Publish the base motion, not only the laser-frame forward component.
  // This matters for JetAuto's omnidirectional lateral motion and for a laser
  // mounted away from the base origin.
  Pose3d robot_increment = robot_oldpose_.inverse() * robot_pose_;
  if (time_inc_sec > 1.0e-6)
  {
    lin_speed = robot_increment.translation()(0) / time_inc_sec;
    lat_speed = robot_increment.translation()(1) / time_inc_sec;
  }
  else
  {
    lin_speed = 0.0;
  ...
  ang_speed = time_inc_sec > 1.0e-6 ? ang_inc/time_inc_sec : 0.0;
