...
public:
  ...
  bool publish_tf, new_scan_available, laser_pose_initialized;

  double freq;
  double pose_covariance_xy, pose_covariance_yaw;
  double twist_covariance_xy, twist_covariance_yaw;
  ...
  GT_pose_initialized(false)
{
  ...
  pn.param<double>("pose_covariance_xy", pose_covariance_xy, 4.0e-4);
  pn.param<double>("pose_covariance_yaw", pose_covariance_yaw, 1.218e-3);
  pn.param<double>("twist_covariance_xy", twist_covariance_xy, 4.0e-4);
  pn.param<double>("twist_covariance_yaw", twist_covariance_yaw, 2.5e-3);
...
{
  if (last_scan.header.frame_id.empty())
  {
    ROS_ERROR("Cannot initialize RF2O laser pose: first scan has no frame_id");
    return false;
  }
  ...
  try
  {
    tf_listener.waitForTransform(base_frame_id, last_scan.header.frame_id,
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::LaserScan::ConstPtr& new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    // The original vendor node queried TF in its constructor, before any
    // LaserScan existed, so the lookup used an empty source frame and silently
    // fell back to an identity extrinsic. Defer initialization until the real
    // scan frame is known and do not process motion without that transform.
    if (!laser_pose_initialized)
    {
      laser_pose_initialized = setLaserPoseFromTf();
      if (!laser_pose_initialized)
      {
        ROS_ERROR_THROTTLE(2.0, "Waiting for RF2O base-to-laser transform");
        return;
      }
    }
...
void CLaserOdometry2DNode::publish()
{
  ...
  odom.twist.twist.linear.y = lat_speed;
  odom.twist.twist.angular.z = ang_speed;   //angular speed
  odom.pose.covariance[0] = pose_covariance_xy;
  odom.pose.covariance[7] = pose_covariance_xy;
  odom.pose.covariance[14] = 1.0e6;
  odom.pose.covariance[21] = 1.0e6;
  odom.pose.covariance[28] = 1.0e6;
  odom.pose.covariance[35] = pose_covariance_yaw;
  odom.twist.covariance[0] = twist_covariance_xy;
  odom.twist.covariance[7] = twist_covariance_xy;
  odom.twist.covariance[14] = 1.0e6;
  odom.twist.covariance[21] = 1.0e6;
  odom.twist.covariance[28] = 1.0e6;
  odom.twist.covariance[35] = twist_covariance_yaw;
  //publish the message
